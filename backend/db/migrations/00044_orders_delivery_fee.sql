-- =============================================================
-- 00044: Valor del domicilio (delivery_fee) — Pago de órdenes DOMICILIO
-- El cajero captura, al pagar una orden de la sala DOMICILIO, el valor del
-- domicilio. Va como LÍNEA APARTE sumada al Total (igual que la propina), NO
-- se mezcla con el subtotal de productos y NO cuenta como venta/facturado
-- (Dashboard/Reportes lo excluyen restándolo de total). Opcional (default 0).
-- Decisiones del usuario (2026-06-13).
-- =============================================================

ALTER TABLE orders ADD COLUMN IF NOT EXISTS delivery_fee NUMERIC(12,2) DEFAULT 0;

-- pay_order ahora recibe el valor del domicilio (7º parámetro) y lo suma al
-- total. Cambia la aridad → se elimina la firma de 6 args (00027/00043) y se
-- crea la de 7 con default 0 (las llamadas de 6 args siguen resolviendo).
DROP FUNCTION IF EXISTS pay_order(INT, INT, NUMERIC, JSONB, INT, UUID);
  ...
  FOR pay IN SELECT * FROM jsonb_array_elements(p_payments) LOOP
    ...
    INSERT INTO order_payments
      (order_id, payment_method_id, bank_id, amount, tip_included,
       change_given, paid_item_ids, voucher_number, user_id)
    VALUES
      (p_order_id,
       (pay->>'method_id')::INT,
  ...
  IF v_pending = 0 THEN
    ...
    SET status = 'pagada', client_id = p_client_id, tip = COALESCE(p_tip, 0),
        ...
        subtotal = (SELECT COALESCE(SUM(subtotal), 0) FROM order_items
                    WHERE order_id = p_order_id AND kitchen_status <> 'cancelado'),
        -- Total = productos no cancelados + propina + DOMICILIO
        total = (SELECT COALESCE(SUM(subtotal), 0) FROM order_items
                 WHERE order_id = p_order_id AND kitchen_status <> 'cancelado')
                + COALESCE(p_tip, 0) + COALESCE(p_delivery_fee, 0),
        cost_total = (SELECT COALESCE(SUM(cost_price * quantity), 0) FROM order_items
                      WHERE order_id = p_order_id AND kitchen_status <> 'cancelado'),
        paid_at = now(),                       -- §1.9.1: fecha de fin de la orden
        updated_at = now()
    WHERE id = p_order_id;
