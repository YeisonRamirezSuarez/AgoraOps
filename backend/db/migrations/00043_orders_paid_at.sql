-- =============================================================
-- 00043: Fecha de fin de la orden (paid_at) — Reportes §1.9.1 (Polaris)
-- El "Reporte general" de Polaris muestra y filtra por la FECHA DE FIN
-- de la orden (dt_order_end): el momento en que la orden quedó pagada/
-- finalizada. hpos no guardaba ese sello (solo opened_at y updated_at),
-- así que se agrega orders.paid_at, se rellena el histórico con
-- updated_at de las órdenes ya pagadas y pay_order lo sella al cerrar.
-- =============================================================

ALTER TABLE orders ADD COLUMN IF NOT EXISTS paid_at TIMESTAMPTZ;

-- Histórico: aproximar la fecha de fin con el último update de las pagadas
UPDATE orders
SET paid_at = updated_at
WHERE status = 'pagada' AND paid_at IS NULL;

CREATE INDEX IF NOT EXISTS idx_orders_tenant_paid ON orders(tenant_id, paid_at);

-- pay_order: sellar paid_at cuando la orden queda totalmente pagada.
-- Misma firma que 00027 (CREATE OR REPLACE, sin DROP).
