-- =============================================================
-- 00027: Usuario que procesa cada pago — Dashboard §1.5 (Polaris)
-- El "Ranking por Pagos" y los KPIs de pagos atribuyen cada cobro
-- al usuario que lo procesó (no al creador de la orden). Se agrega
-- order_payments.user_id, se rellena el histórico con el usuario de
-- la orden y pay_order recibe quién cobra.
-- =============================================================

ALTER TABLE order_payments ADD COLUMN user_id UUID REFERENCES users(id);

-- Histórico: atribuir al usuario que atendió la orden
UPDATE order_payments op
SET user_id = o.user_id
FROM orders o
WHERE o.id = op.order_id AND op.user_id IS NULL;

CREATE INDEX idx_order_payments_user ON order_payments(user_id);

DROP FUNCTION IF EXISTS pay_order(INT, INT, NUMERIC, JSONB, INT);
