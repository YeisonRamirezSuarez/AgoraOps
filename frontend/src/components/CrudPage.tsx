...
import {
  Button, ConfirmDialog, Field, Input, Modal, Select, useToast,
} from "./ui";

export interface CrudField {
  name: string;
  label: string;
  type?: "text" | "number" | "select" | "checkbox" | "datetime-local";
...
export function CrudPage({
  ...
  return (
    <div className="fade-in-up">
      ...
      {useModal && (
        <Modal open={!!editing} title={isNew ? `Agregar ${title}` : `Editar ${title}`}
          ...
          {editing && (
            <form onSubmit={(e) => { e.preventDefault(); save(); }} className="space-y-4">
              {fields.map((f) => {
                const disabled = !isNew && f.immutable;
                return (
                  <Field key={f.name} label={f.label + (disabled ? " (no editable)" : "")}>
                    {f.type === "select" ? (
